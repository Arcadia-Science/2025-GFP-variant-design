...
def generate_embeddings(fasta_path, toks_per_batch, repr_layers, model, output_dir, truncation_seq_length=1022):
    alphabet = model.alphabet
    data_loader = data_loader_for_fasta(fasta_path, toks_per_batch, alphabet)
    num_batches = len(data_loader.batch_sampler)

    assert all(-(model.num_layers + 1) <= i <= model.num_layers for i in range(repr_layers))
    repr_layers = [(i + model.num_layers + 1) % (model.num_layers + 1) for i in range(repr_layers)]
    ...
    with torch.no_grad():
        for batch_idx, (labels, strs, toks) in enumerate(data_loader):
            ...
            print(f"Processing {current} of {num_batches} batches ({seqs_in_batch} sequences)")
